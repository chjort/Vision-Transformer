...
class Dataset:
    ...
    def map(self, func):
        self.dataset = self.dataset.map(func, num_parallel_calls=self.n_parallel)
    ...
    def interleave(self, map_func, cycle_length=-1, block_length=1):
        self.dataset = self.dataset.interleave(map_func, cycle_length, block_length, num_parallel_calls=self.n_parallel)
